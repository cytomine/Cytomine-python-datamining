...
class WorkflowExecutor(Loggable):
    ...
    def __init__(self, workflow, mute_workflow=True, logger=SilentLogger()):
        ...
        mute_workflow: bool
            True for disabling the workflow logging during execution on sub-images
        logger: Logger (optional, default: a SilentLogger instance)
            A logger object
        """
        Loggable.__init__(self, logger)
        self._workflow = workflow
        self._mute_workflow = mute_workflow
    ...
    def execute(self, image, workflow_info_collection):
        ...
        for sub_image in images:
            ...
            if self._mute_workflow:
                self._workflow.logger.level = previous_verbosity
    ...
    def get_windows(self, image, workflow_info_collection):
        """Given the workflow_info_collection produces by execution of sldc workflows, produces images objects for the
        next execution
...
class FullImageWorkflowExecutor(WorkflowExecutor):
    """A workflow executor which simply process the full image it is passed and doesn't post-process the generated data.
    """
    def __init__(self, workflow, mute_workflow=False, logger=SilentLogger()):
        WorkflowExecutor.__init__(self, workflow, mute_workflow=mute_workflow, logger=logger)
...
class PostProcessor(Loggable):
    """A post processor is a class encapsulating the processing of the results of several SLDCWorkflow executions.
    """
    __metaclass__ = ABCMeta
...
class WorkflowChain(Loggable):
    ...
    def __init__(self, image_provider, executors, post_processor, logger=SilentLogger()):
    ...
    def execute(self):
        """Execute the workflow chain over the images generated by the image provider
        """
    ...
    def _process_image(self, image, image_nb):
        ...
        image_nb: int
            The number of the image to be processed
        """
