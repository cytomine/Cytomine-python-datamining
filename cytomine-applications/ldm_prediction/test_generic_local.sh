...
model_names='first_model,second_model'  #name of the models built with the model builder, e.g 'first_model,second_model,third_model,fourth_model,fifth_model'
verbosity=0 #verbosity of the cytomine client, e.g 0
image_predict='6603457,6603463,6603649'

#3. Run this script (sh predict.sh)

python landmark_generic_predict.py --cytomine_host $host --cytomine_public_key $public_key --cytomine_private_key $private_key --cytomine_id_software $id_software --cytomine_base_path $base_path --cytomine_working_path $working_path --cytomine_id_project $id_project --cytomine_predict_images $image_predict --model_load_from $model_path --model_names $model_names --image_type $image_type --verbose $verbosity