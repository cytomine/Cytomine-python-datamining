...
from .datatype.bounds import Bounds
...
class ThreadUrl(threading.Thread):
    def __init__(self, queue, out_queue, cytomine, terminate_event, verbose = False):
...
class CytomineReader(Reader):
    ...
    def read(self, async = False):
        ...
        row1 = int(math.floor(min(self.image.levels[self.zoom]['level_height'], (self.window_position.y + self.window_position.height) / self.image.tile_size)))
        col1 = int(math.floor(min(self.image.levels[self.zoom]['level_width'], (self.window_position.x + self.window_position.width) / self.image.tile_size)))
        cols = col1 - col0 + 1
        rows = row1 - row0 + 1
...
#                url = "%sTileGroup%d/%d-%d-%d.jpg" % (base_url, tile_group, self.image.depth - self.zoom, col, row) #old syntax
                url = "%s&tileGroup=%d&z=%d&x=%d&y=%d&mimeType=%s" % (base_url, tile_group, self.image.depth - self.zoom, col, row, self.image.mime)
                url = url.replace(" ", "%20")
    ...
    def read_window(self):
        ...
        url = "%s%s%s%s?zoom=%d" % (self.cytomine._protocol, self.cytomine._host, self.cytomine._base_path,self.image.getCropURL(window), self.zoom)
