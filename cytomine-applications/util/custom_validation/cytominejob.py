# -*- coding: utf-8 -*-
"""
Copyright 2010-2013 University of LiÃƒÂ¨ge, Belgium.
...
__copyright__ = "Copyright 2010-2013 University of LiÃƒÂ¨ge, Belgium"
__version__ = '0.1'
...
class CytomineJob:
    ...
    def set_progress(self, progress, comment):
        """
        Notify the Cytomine server of a progress in the job execution
        Incurs a dataflow
        :param progress: The progress (in [0;100])
        :param comment: The status comment
        """
        if not 0 <= progress <= 100:
            raise ValueError("Progress value should be between 0 and 100 (both inclusive).")
        job = self.__job
        self.__job = self.__cytomine.update_job_status(job, status_comment=comment, progress=progress)

    def add_parameters(self, parameters):
        """
        Set the parameters for the given jov
        :param parameters: The software parameters to add
        :return:
        """
        software = self.__cytomine.get_software(self.software_id)
        return self.__cytomine.add_job_parameters(self.__job.id, software, parameters)
